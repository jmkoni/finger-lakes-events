...
function createEndOfFileLocation(source) {
    ...
    return core.Location.from(lastLineNumber, startColumn, lastLineNumber, lastColumnNumber);
}
/**
...
class ERBNoCaseNodeChildrenVisitor extends BaseRuleVisitor {
    ...
    checkCaseNodeChildren(node, type) {
        ...
        const conditionCode = firstCondition && core.isERBNode(firstCondition) ? IdentityPrinter.printERBNode(firstCondition) : `<% ${type} ... %>`;
        for (const child of node.children) {
    ...
    isAllowedContent(node) {
        if (core.isWhitespaceNode(node))
            return true;
        if (core.isCommentNode(node))
            return true;
        if (core.isLiteralNode(node) || core.isHTMLTextNode(node)) {
...
function positionFromOffset(source, offset) {
    ...
    return new core.Position(line, column);
}
class ERBNoExtraNewLineVisitor extends BaseSourceRuleVisitor {
    visitSource(source) {
        ...
        while ((match = regex.exec(source)) !== null) {
            ...
            const location = new core.Location(start, end);
...
class ERBNoExtraWhitespaceInsideTagsVisitor extends BaseRuleVisitor {
    ...
    getWhitespaceLocation(node, content, position, offset = 0) {
        ...
        if (position === "start") {
            ...
            return core.Location.from(contentLocation.start.line, startColumn, contentLocation.start.line, startColumn + length);
        }
        else {
            const match = content.match(/\s+$/);
            const length = match ? match[0].length : 0;
            return core.Location.from(contentLocation.end.line, contentLocation.end.column - length, contentLocation.end.line, contentLocation.end.column);
        }
    }
...
class HerbDisableCommentBaseVisitor extends BaseRuleVisitor {
    ...
    createRuleNameLocation(node, ruleDetail) {
        ...
        return core.Location.from(startLine, startColumn, startLine, startColumn + ruleDetail.length);
    }
    /**
    ...
    checkParsedHerbDisable(node, _content, herbDisable) {
        herbDisable.ruleNameDetails.forEach(ruleDetail => {
            if (this.validRuleNames.has(ruleDetail.name))
                return;
            const suggestion = core.didyoumean(ruleDetail.name, this.validRuleNamesList);
...
class HTMLAttributeValuesRequireQuotesRule extends ParserRule {
    ...
    autofix(offense, result, _context) {
        ...
        if (!value)
            return null;
        const quote = core.Token.from({ type: "TOKEN_QUOTE", value: '"', location: core.Location.zero, range: [0, 0] });
        if (value.open_quote) {
            value.open_quote.value = '"';
        }
        else {
...
class HTMLNoDuplicateMetaNamesVisitor extends ControlFlowTrackingVisitor {
    ...
    extractAttributes(node, metaTag) {
        if (core.isHTMLElementNode(node) && node.open_tag) {
 ...
 * - ERB output tags (<%= %>, <%== %>)
 */
class ContainsOutputContentVisitor extends core.Visitor {
...
class HTMLNoSelfClosingRule extends ParserRule {
    ...
    autofix(offense, result, _context) {
        ...
        if (node.children && Array.isArray(node.children)) {
            ...
            if (children.length > 0 && core.isWhitespaceNode(children[children.length - 1])) {
        ...
        if (!isVoid) {
            ...
            if (parent && parent.type === "AST_HTML_ELEMENT_NODE") {
                const tag_opening = { type: "TOKEN_HTML_TAG_START_CLOSE", value: "</", location: core.Location.zero, range: [0, 0] };
                const tag_name = { type: "TOKEN_IDENTIFIER", value: tagName, location: core.Location.zero, range: [0, 0] };
                const tag_closing = { type: "TOKEN_HTML_TAG_END", value: ">", location: core.Location.zero, range: [0, 0] };
                parent.close_tag = core.HTMLCloseTagNode.from({
...
class HTMLNoSpaceInTagVisitor extends BaseRuleVisitor {
    ...
    checkWhitespaceInSingleLineTag(children, isSelfClosing) {
        const whitespaceNodes = core.filterWhitespaceNodes(children);
    ...
    checkMissingSpaceBeforeSelfClosing(node, children, isSelfClosing) {
        ...
        if (lastChild && core.isWhitespaceNode(lastChild))
            return;
        const lastNonWhitespace = children.filter(child => !core.isWhitespaceNode(child)).pop();
    ...
    checkMultilineTag(node) {
        const whitespaceNodes = core.filterWhitespaceNodes(node.children);
    ...
    reportAllWhitespace(nodes, message) {
        const whitespaceNodes = Array.isArray(nodes) && nodes.length > 0 && !core.isWhitespaceNode(nodes[0])
            ? core.filterWhitespaceNodes(nodes)
            : nodes;
        whitespaceNodes.forEach(whitespace => {
...
class HTMLNoSpaceInTagRule extends ParserRule {
    ...
    autofix(offense, result, _context) {
        ...
        if (!node)
            return null;
        if (core.isHTMLOpenTagNode(node)) {
            const token = core.Token.from({ type: "TOKEN_WHITESPACE", value: " ", range: [0, 0], location: core.Location.zero });
            const whitespace = new core.WhitespaceNode({ type: "AST_WHITESPACE_NODE", value: token, location: core.Location.zero, errors: [] });
        ...
        switch (message) {
            case MESSAGES.EXTRA_SPACE_NO_SPACE: {
                ...
                const parent = findParent(result.value, node);
                if (parent && core.isHTMLOpenTagNode(parent)) {
...
exports.DOCUMENT_ONLY_TAG_NAMES = DOCUMENT_ONLY_TAG_NAMES;
...
exports.HTMLBodyOnlyElementsRule = HTMLBodyOnlyElementsRule;
exports.HTMLBooleanAttributesNoValueRule = HTMLBooleanAttributesNoValueRule;
exports.HTMLHeadOnlyElementsRule = HTMLHeadOnlyElementsRule;
exports.HTMLIframeHasTitleRule = HTMLIframeHasTitleRule;
exports.HTMLImgRequireAltRule = HTMLImgRequireAltRule;
exports.HTMLInputRequireAutocompleteRule = HTMLInputRequireAutocompleteRule;
...
exports.HTMLNoDuplicateMetaNamesRule = HTMLNoDuplicateMetaNamesRule;
...
exports.HTMLNoSpaceInTagRule = HTMLNoSpaceInTagRule;
...
exports.HerbDisableCommentBaseVisitor = HerbDisableCommentBaseVisitor;
exports.HerbDisableCommentMalformedRule = HerbDisableCommentMalformedRule;
exports.HerbDisableCommentMissingRulesRule = HerbDisableCommentMissingRulesRule;
exports.HerbDisableCommentNoDuplicateRulesRule = HerbDisableCommentNoDuplicateRulesRule;
exports.HerbDisableCommentNoRedundantAllRule = HerbDisableCommentNoRedundantAllRule;
exports.HerbDisableCommentParsedVisitor = HerbDisableCommentParsedVisitor;
exports.HerbDisableCommentUnnecessaryRule = HerbDisableCommentUnnecessaryRule;
exports.HerbDisableCommentValidRuleNameRule = HerbDisableCommentValidRuleNameRule;
