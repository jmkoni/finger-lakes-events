...
var tailwindClassSorter$1 = require('@herb-tools/tailwind-class-sorter');
...
class CustomRewriterLoader {
     ...
     */
    async loadRewriterFile(filePath) {
        try {
        ...
        for (const filePath of rewriterFiles) {
            ...
            for (const RewriterClass of rewriters) {
                ...
                if (seenNames.has(rewriterName)) {
                    ...
                    duplicateWarnings.push(`Custom rewriter "${rewriterName}" is defined in multiple files: "${firstPath}" and "${filePath}". The later one will be used.`);
                }
                else {
     ...
     */
    static async hasCustomRewriters(baseDir = process.cwd()) {
     ...
     */
    static async loadAndMergeRewriters(builtinRewriters, options = {}) {
...
class TailwindClassSorterVisitor extends Visitor {
    ...
    formatSortedClasses(literals, others, preserveLeadingSpace, isNested) {
        if (literals.length === 0 && others.length === 0)
            return [];
    ...
    formatSortedLiteral(literal, fullContent, sortedClasses, trimmedClasses) {
    ...
    formatSortedLiteralWithERB(literal, fullContent, sortedClasses, others, preserveLeadingSpace, isNested) {
        const leadingSpace = fullContent.match(/^\s*/)?.[0] || "";
        const trailingSpace = fullContent.match(/\s*$/)?.[0] || "";
    ...
    addSpacingBetweenERBNodes(nodes, isNested, trailingSpace) {
 ...
 */
class TailwindClassSorterRewriter extends ASTRewriter {
    sorter;
    get name() {
    ...
    async initialize(context) {
        try {
            this.sorter = await tailwindClassSorter$1.TailwindClassSorter.fromConfig({
            ...
            if (errorMessage.includes('Cannot find module') || errorMessage.includes('ENOENT')) {
                throw new Error(`Tailwind CSS is not installed in this project. ` +
 ...
 */
function getBuiltinRewriter(name) {
 ...
 */
function getBuiltinRewriterNames() {
 ...
 */
async function tailwindClassSorter(options = {}) {
...
exports.builtinRewriters = builtinRewriters;
exports.getBuiltinRewriter = getBuiltinRewriter;
exports.getBuiltinRewriterNames = getBuiltinRewriterNames;
...
exports.tailwindClassSorter = tailwindClassSorter;
